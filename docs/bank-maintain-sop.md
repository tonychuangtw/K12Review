...
⛔ **剛加完題、剛重建完，不要跑同步**：build-bank 的答案位置旋轉是「照題目順序」算的，
把旋轉後的結果寫回 jsonl 再建一次，位置會再轉一次（不是冪等的）。
同步只用在「直接改了 js/data」之後；加題走的是 jsonl → build-bank 這個方向，本來就不會分岔。

⚠ 跑重建指令時 `$PF` 這類變數要跟指令在**同一個 shell** 裡：
`awk ... | bash` 會讓變數展不開，build-bank 直接 ENOENT 失敗，
而失敗時 js/data 不會被改動——「檔案沒變」看起來就跟「重建結果一致」一模一樣，很容易誤判成驗證通過。
所以驗收一定要**親眼看到 `<科> NNNN 題 → js/data/<科>.js` 這一行**。
