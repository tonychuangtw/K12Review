...
## 教科書版本（Tony 2026-08-28 提供，編課本相關內容一律照這個）
...
生字表／課次來源：教育部教育雲「生字詞彙表」
`https://pedia.cloud.edu.tw/Bookmark/Textword?year=115_1&degree=<年級>&subject=國語文&press=<版本>`
→ 取課次 id 後 `Bookmark/TCollection?TextNameId=<id>` 取該課生字與語詞。
已抓好的存在 `docs/source/`（目前有康軒四上）。**題目一律自撰，不抄課本內容。**
...
- 改到互動元件（js/widgets.js）或概念卡的 viz 設定，跑 `node test/widget-audit.mjs`：
...
- **示意圖用向量、整張地圖當主角的卡才用 AI 插圖**（2026-08-29 Tony：線條圖旁邊貼 AI 插畫「會很怪」）。
  示意圖裡要放臺灣就呼叫 `drawTaiwan(svg, x, y, w, h)`（`js/widgets.js`，座標取自實際海岸線經緯度）
- ⚠ **圖上只要提到東西南北，一律用「上北下南、左西右東」，並呼叫 `compassRose(svg, x, y)` 畫方位標**
  （2026-08-29 Tony：颱風卡寫「常見路徑往西北」，臺灣卻畫在右邊，箭頭指過去變成往東北）。
  同一輪修正：冷鋒三角形要指向前進方向（往南＝往下）、地球自轉圖面向太陽的那半邊才是白天
- **課文帶讀**（`js/data/texts-<科>.js`，`APP_TEXTS`）：單元學習的第一段，流程是
  課文帶讀 → 概念卡 → 單元測驗。每段要有小標、3 句以上的短句（每句 ≤60 字，才唸得完）、
  一題「讀懂了嗎」（答對才解鎖下一段），選填 terms／viz。⛔ **課文一律自撰，不可抄課本原文**（版權）。
  改完跑 `node test/test.js`（資料守門）與 `node test/browser-smoke.mjs`（第 11 節走完整個帶讀流程）
...
- 新增一堂之後跑 `node test/test.js`（資料守門）與 `node test/browser-smoke.mjs`（走完日曆→複習→測驗→精熟度）

## 2026-08-20 導覽與命名（Tony 五點回報後定案）

- **匯入題庫是完整獨立的一區**（2026-08-29 Tony）：自己的錯題本（`wb.scope='import'`，可依科目／年級／冊課／難度篩）、
  自己的進度分析（`showImportProgress()`），家長／老師儀表板也有專屬區塊。判斷「是不是匯入題庫」用 `isImportCat()`，
  不要用 `isBankCat()`（後者含各科自編原創題庫）
