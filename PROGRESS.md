...
NEXT_ACTION: 中醫師 168 卷已全數完成。接著照 Tony 2026-09-08 定的順序跑「申論題 AI 批改」小實驗——取考選部有公布評分標準的律師二試題（先去考選部網站抓題與命題大意及評分標準），讓 Kimi K3／Haiku 4.5／Sonnet 5／Opus 各批同一批答案，比對爭點抓取與分數落點誰最接近標準，預算台幣 100 元內；結果寫進 kaoguhero/docs/plan-log.md。實驗做完再開始藥師詳解（pha-*，12,600 題，每卷寫完都要跑 node tools/build-index.js --write）
