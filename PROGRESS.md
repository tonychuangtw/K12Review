...
NEXT_ACTION: 【歷屆學測自然】110～103 八卷已完成。**接著做 102、101、100 三卷就收工**（2026-09-02 Tony：各科都做到 100 年後的就好）（subj science；111 起自然 44 題約 96 分、110 以前 68 題 128 分，逐卷看卷首配分）。國英數社四科 109 卷 4,528 題已收齊（年份下限 90）。自然做完接會考數英社自（115-103）、基測（102-90）。流程照舊：
 ...
 (9) 歷屆學測（2026-09-01 Tony 新指示：「所有學測題另外做一個大項。進去可選哪一年哪一科直接做全部。
     ...
     【總進度表】（做完一項就把 ⬜ 改成 ✅，並補上卷數／題數）
       ...
       學測 自然：✅115（48 題/96 分）✅114（48 題/96 分）✅113（48 題/96 分）✅112（52 題/104 分）✅111（52 題/104 分）
                ...
                ⬜102 ⬜101 ⬜100（做到 100 為止，見下方 2026-09-02 Tony 範圍指示）
       ...
       【下一步：102 自然】試卷與答案尚未下載。大考中心列表頁 page=8 就有 102 的連結
         （`https://www.ceec.edu.tw/xmfile?xsmsid=0J052424829869345634&page=8`，抓 title 含「102學測自然」
         的 a href）。做法完全照 103-110：pdftotext 抽文字 → 只裁圖表 → 分批寫解析 → build.py 對答案
         → 註冊 exams.js → node test/test.js → stamp → push。
