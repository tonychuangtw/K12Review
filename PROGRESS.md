...
NEXT_ACTION: 【歷屆學測自然】110 做到一半：裁圖 21 檔已完成、卷面結構已查證、答案 1-34 題已驗證，**從第 35 題續讀（先用 pdftotext 抽出的 n110/raw.txt，圖題才翻 p11.png）**（詳見總進度表下方的存檔段落，接續即可），完成後 109→90 逐年做（subj science；111 起自然 44 題約 96 分、110 以前 68 題 128 分，逐卷看卷首配分）。國英數社四科 109 卷 4,528 題已收齊（年份下限 90）。自然做完接會考數英社自（115-103）、基測（102-90）。流程照舊：
...
【原本的八項清單，其餘照舊】
 ...
 (9) 歷屆學測（2026-09-01 Tony 新指示：「所有學測題另外做一個大項。進去可選哪一年哪一科直接做全部。
     ...
     【總進度表】（做完一項就把 ⬜ 改成 ✅，並補上卷數／題數）
       ...
       【110 自然做到一半（2026-09-02 09:0x 額度中斷前存檔，第二次）】：
       - 試卷/答案已下載：舊 scratchpad dl/110自然試卷.pdf、110自然答案.pdf；20 頁已渲染成
         `<舊scratchpad>/n110/p1.png…p20.png`（路徑 /tmp/claude-1000/-home-tony-TelegramClaude-chinese/
         ecad1288-0839-4cc5-9d62-1b4ee90da0b5/scratchpad）。若被清掉就重下（p5.html 的 CEEC 連結，
         或 xmfile page=5 搜「110學測自然」），再 tools/exam-crop.py page 1-20 重畫。
         ⚠ 對照關係：pN.png ＝ 卷面第 (N-1) 頁（p1 是封面）。
       - ✅ **裁圖已全部完成**（21 檔，未 commit，在 img/exam/110-science/）：
         q4 q7 q11 q15 q16 q17 q21 q22 q28 q34 q35 q41 q45 q46 q53 q56 q62 q65 q68、g51、g60
       - 格式（舊制，✅ 已翻卷首查證，取代先前的猜測）：考試時間 **110 分鐘**、68 題全選擇。
         第壹部分 80 分＝一、單選 1-22（占 44 分，每題 2 分）／二、多選 23-36（占 28 分，每題 2 分）／
         三、綜合題 37-40（占 8 分，每題 2 分，單選多選混編）。
         ⚠ **多選一律用通式 (n−2k)/n 計分，沒有「只錯一個得 1 分」的 pk 規則**（卷面說明白紙黑字），
           所以 q 不用寫 pk，照 111-115 的寫法即可。
         第貳部分 41-68（每題 2 分）。max 取 136（本站不設原卷上限，note 寫明原卷第貳部分上限 48 分）。
       ...
       - ✅ 已讀卷並自行驗證答案：**第 1-34 題全數吻合**（p1-p10）。重算過的關鍵題：
         Q9 生成熱用 ΔHf=−2040−ΔHc 推；Q16 走時曲線 P 0.2 s/km、S 1/3 s/km，差 8 秒 → 60 km；
         Q27 設 x=C2H4、y=H2，28x+2y=94 且反應後總莫耳＝y=5 → x=3（故 C、D 對）；
         Q34 車過 O 時發聲，c、d 在垂直方向 → f_c=f_d=f_0，且 f_b>f_0>f_a（故 C、E 對）；
         Q32 官方雙答案（BD 或 BC）—— 管內原磁場向上（B）確定，強度增/減官方併給分，寫成 q.alt。
       - ★ **2026-09-02 新發現（後續 109→90 自然卷一律照做）：110 自然的 PDF `pdftotext -layout` 抽得出全卷文字**，
         題幹與選項都完整（只有上下標、公式符號會走樣，圖當然抽不出來）。所以**不必逐頁看圖讀題**，
         流程改成：pdftotext 出全文當題幹來源 → 只在「有圖／有公式」的題翻對應的 pN.png 確認 →
         寫 JSON。抽出來的檔已存在 `<舊scratchpad>/n110/raw.txt`（950 行，已把連續空白壓成單一空白；
         CJK 字元之間仍有排版空格，寫進 JSON 前要自己去掉）。指令：
         `pdftotext -layout "<舊scratchpad>/dl/110自然試卷.pdf" raw.txt`
         （數學卷仍然不行——那是 wmf 公式；自然卷可行是因為題幹以文字為主。）
       - 下一步：**從第 35 題續讀（raw.txt 對應段落，圖題再翻 p11.png 以後）**（35、36 題幹已讀到：Q35 圖 10 年降雨量，答 AD；Q36 海流成因，答 AE），
         接著 p12→p20 讀 37-68 題，逐題驗證 → 照 111-115 的批次寫法在 scratchpad 開 n110w/ 寫 body 腳本
         → build 出 js/data/exam/110-science.js → exams.js 註冊 `{ id:'110-science', year:110, subj:'science',
         n:68, max:136, mins:110 }` → `node test/test.js` → `python3 tools/stamp-version.py` → commit → 回報 Tony。
         之後 109→90 逐年同法（皆舊制 68 題，但每年配分／題數要各自翻卷首確認，不要照抄 110）。
