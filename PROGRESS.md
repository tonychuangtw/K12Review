...
NEXT_ACTION: 【歷屆學測自然】110、109 已完成。**108 進行中**（試卷/答案已下載、raw.txt 已抽、答案已逐題自行驗證，見下方存檔），接著 107→90 逐年做（subj science；111 起自然 44 題約 96 分、110 以前 68 題 128 分，逐卷看卷首配分）。國英數社四科 109 卷 4,528 題已收齊（年份下限 90）。自然做完接會考數英社自（115-103）、基測（102-90）。流程照舊：
 ...
 (9) 歷屆學測（2026-09-01 Tony 新指示：「所有學測題另外做一個大項。進去可選哪一年哪一科直接做全部。
     ...
     【總進度表】（做完一項就把 ⬜ 改成 ✅，並補上卷數／題數）
       ...
       學測 自然：✅115（48 題/96 分）✅114（48 題/96 分）✅113（48 題/96 分）✅112（52 題/104 分）✅111（52 題/104 分）
                ✅110（68 題/136 分）✅109（68 題/136 分）⬜108-90（皆舊制，配分逐年翻卷首確認）
       ...
       【108 自然做到一半（2026-09-02 存檔）】
       - 試卷／答案已下載：`<舊scratchpad>/dl/108自然試卷.pdf`、`108自然答案.pdf`（CEEC list6.html 有連結）
       - 全卷文字已抽：`<舊scratchpad>/n108/raw.txt`（1020 行，pdftotext -layout）
       - 卷面結構（已翻卷首）：單選 1-16（32 分）／多選 17-34（36 分）／綜合題 35-40（12 分）／
         第貳部分 41-68（48 分）＝68 題 136 分、110 分鐘。多選一律通式，不設 pk。
       - 官方答案（已轉錄）：1B 2C 3D 4B 5D 6E 7D 8D 9C 10E 11B 12B 13E 14C 15A 16B
         17ACD 18BD 19BD 20CE 21AD 22BCE 23CE 24AC 25CD 26AD 27ADE 28BC 29ACE 30AE
         31BCD 32AD 33ABD 34BCE 35B 36A 37D 38A 39D 40CE
         41CD 42BDE 43BD 44D 45A 46A 47BCE 48E 49E 50C 51A 52BD 53D 54C 55B 56A 57B 58D 59D 60D
         61BCD 62BE 63D 64E 65A 66C 67BC 68ADE
       - ✅ 答案已逐題自行推導驗證（僅 Q4/10/12/13/20/25/26/33/37/41/43/45/46/48/50/57/59/60 這些
         純圖形題要等裁圖後才能寫誘答說明，其餘全部算過且吻合）。關鍵重算：
         Q14 等莫耳燃燒耗氧 乙烷3.5>甲醚=乙醇3>乙炔2.5>乙酸2；Q16 OH⁻0.02mol→C₂H₂ 0.26g；
         Q34 由 Q 佔 20% 解出 R=12q ⟹ 甲=C₂H₆、乙=C₂H₄，x=3,y=2,z=2；Q39 n/N∝1/r ⟹ 10⁵ 倍；
         Q44 應變率 (0.08/250000)/3.15e7 ≈ 1e-14；Q47 抬升 1 km 達飽和(20℃)、乙地 15℃、丙地 35℃；
         Q51 T²∝r³ ⟹ r 比 1/4 ⟹ 重力 16 倍；Q53 200nm/0.106nm≈2000≈1000 倍；
         Q55 (1264−1217)/5 = 9.4 百萬 m²；Q56 1000×30×10×160×0.25 = 12 MW；Q65 烯與酯共 2 類
       - 下一步：裁圖（約 27 張，清單見下）→ 寫 68 題解析 → build → 註冊 exams.js → test → commit
         需裁圖的題：q4 圖1(q7) 圖2(q10) 圖3(q12) q13 圖4(q20) 圖5(q25) 圖6(q26) 圖7(q28) 圖8(q30)
         圖9(q32) 圖10(q33) 圖11(q37) 圖12(q38) 圖13(q40) 圖14(g41) 圖15(g43) 圖16(g45)＋q45選項
         圖17(q47) 圖18(q48)＋選項 圖19(q49) 圖20(g54) 圖21(q57) 圖22(q59)＋選項 q60 圖23(q62) 圖24(g66)
       【110 自然已完成 ✅ 2026-09-02，68 題／136 分，commit 2e09d1c0】做法留給 109→90 照抄：
       - 舊制卷面結構（已翻卷首查證）：考試時間 110 分鐘、68 題全選擇。
         第壹部分 80 分＝一、單選 1-22（44 分，每題 2 分）／二、多選 23-36（28 分）／
         三、綜合題 37-40（8 分，單選多選混編）；第貳部分 41-68（每題 2 分，原卷上限 48 分）。
         max 取 136（本站不設原卷上限，note 寫明）。
         ⚠ 多選一律用通式 (n−2k)/n，**不寫 pk**（卷面說明白紙黑字）。
         ⚠ 第 32 題大考中心公告 BD／BC 一併給分 → 用 q.alt（多選的 alt 是「陣列的陣列」）。
       - ★★ **自然卷讀題流程（2026-09-02 定案，109→90 一律照做，這是省 token 的關鍵）**：
         1. `pdftotext -layout <試卷.pdf> raw.txt` —— 自然卷抽得出全卷文字（題幹＋選項完整，
            只有上下標、數學符號會走樣，圖抽不出）。**不要再一頁一頁看 png 讀題**。
         2. 只在「有圖／有數學符號」的題翻對應的 pN.png 或裁好的小圖確認。
            110 這卷實際只翻了 3 張整頁（p4 單位式、p10 都卜勒關係式、p18 終端速率與滑輪等式）
            ＋ 9 張裁圖，其餘全靠 raw.txt。
         3. 裁圖仍照舊：`tools/exam-crop.py page` 出整頁抓座標 → `crop` 出圖。
         ⚠ 數學卷不適用（公式是 wmf），仍要逐頁看圖。
       - 寫題流程：scratchpad 開 n<年>w/，分批寫 hA/hB/hC*.json（每批 14-22 題，一個 python 檔），
         再用 build.py 合併：**build.py 內建官方答案表 KEY 做逐題比對**（不符就 raise，0 筆不符才寫檔），
         同時檢查 ✅／❌／📚 三段解析齊全、選項數、題組存在、附圖檔存在。
         → 註冊 js/data/exams.js → `node test/test.js` → `python3 tools/stamp-version.py` → commit → 回報。
       - 110 這卷需要靠圖才判得出答案的題：q4（分子示意圖＝2H₂O₂→2H₂O＋O₂）、q7（球棍模型，醇類 2 個）、
         q17（聲速剖面先減後增）、q21（凡那比颱風雨區 乙丙甲）、q45（三樣區＝針闊混合／高山針葉／低海拔闊葉）、
         q65（南太平洋高水位＋西側等高線密集）、g51（七個 C10 萜類比分子式）。
