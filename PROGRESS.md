...
NEXT_ACTION: 【歷屆學測自然】110、109、108 已完成。**107 進行中**（已下載、raw.txt 已抽、68 題答案已逐題自行驗證，見下方存檔；下一步＝裁約 24 張圖 → 寫解析）。之後 106→90 逐年做（subj science；111 起自然 44 題約 96 分、110 以前 68 題 128 分，逐卷看卷首配分）。國英數社四科 109 卷 4,528 題已收齊（年份下限 90）。自然做完接會考數英社自（115-103）、基測（102-90）。流程照舊：
 ...
 (9) 歷屆學測（2026-09-01 Tony 新指示：「所有學測題另外做一個大項。進去可選哪一年哪一科直接做全部。
     ...
     【總進度表】（做完一項就把 ⬜ 改成 ✅，並補上卷數／題數）
       ...
       學測 自然：✅115（48 題/96 分）✅114（48 題/96 分）✅113（48 題/96 分）✅112（52 題/104 分）✅111（52 題/104 分）
                ✅110（68 題/136 分）✅109（68 題/136 分）✅108（68 題/136 分）⬜107-90（皆舊制，配分逐年翻卷首確認）
       ...
       【107 自然做到一半（2026-09-02 存檔）】
       - 檔案：`<舊scratchpad>/dl/107自然試卷.pdf`、`107自然答案.pdf`、`<舊scratchpad>/n107/raw.txt`（774 行）
         下載連結在 CEEC list6.html（`grep -o 'href="[^"]*107%e5%ad%b8%e6%b8%ac%e8%87%aa%e7%84%b6[^"]*"'`）
       - 卷面結構（已翻卷首）：**考試時間 100 分鐘**（與 108-110 的 110 分鐘不同！）、16 頁 PDF、
         單選 1-23（46 分）／多選 24-36（26 分）／綜合題 37-40（8 分）／第貳部分 41-68（48 分）＝68 題 136 分
       - ⚠ 第 15、32、52 題是 (A)-(F) 六選項（build.py 的選項數檢查要開例外）
       - 官方答案（已轉錄）：1C 2A 3B 4D 5E 6C 7B 8B 9A 10D 11C 12E 13B 14A 15E 16E 17A 18C 19A 20A
         21C 22C 23A 24CD 25ABE 26AD 27ADE 28CD 29BC 30BE 31AC 32BE 33CE 34ACD 35BD 36ABD 37E 38B 39A 40AE
         41E 42D 43A 44D 45BCD 46ABC 47AE 48ADE 49BE 50C 51BCE 52F 53A 54CE 55D 56C 57A 58E 59E 60A
         61BD 62BDE 63B 64B 65AE 66D 67CE 68BE
       - ✅ 68 題答案已逐題自行推導驗證（純圖形題待裁圖後補誘答說明）。已算過的關鍵題：
         Q1 由「原子序和 25＋表 1 位置」解出 Y=C(6)、Z=O(8)、X=Na(11) ⟹ 丙丁；
         Q4 甲醇 2 H₂O／乙醇 3 H₂O ⟹ 2:3；Q8 3Cu+8HNO₃→3Cu(NO₃)₂+4H₂O+2NO ⟹ NO 係數 2；
         Q14 放大率 2 ⟹ 實際 15cm/5s=3cm/s，f=3Hz ⟹ λ=1.0cm；
         Q28 1.5e11/(2×86400)=8.7e5 m/s；Q37 ᵇₐn 的 b 是質量數 ⟹ a=0,b=1,c=5,x=1,y=1 ⟹ 乙丁；
         Q42/43 由丙的規則：4+4+3+2=13，連 3 根單鍵各減 2 ⟹ 最多 n=7、最少 n=1；
         Q44 各離子×銀離子比：CrO₄²⁻ 0.4M×10mL×2=8 mmol 最多；
         Q55 k(d₁−d₀)+k(d₀−d₂)=Mg ⟹ g=(d₁−d₂)k/M；Q58 虧損 3M⊙=6e30 kg ⟹ E=mc²=5.4e47 J；
         Q59 z=0.1 由圖 21 讀 d≈1.3e9 光年 ⟹ 1.3e9 年；
         Q60 (800/100)²=64 ⟹ r 比 4 ⟹ r'=28480 km ⟹ 高度 ≈22000 km；
         Q61 動量守恆 2m甲v₀=m乙(v₀−v₁) ⟹ m甲<m乙；Q64 朔望月 29.53 天 > 2 月天數 ⟹ 2 月無藍月；
         Q65 半個近點月≈13.66 天 ⟹ 1/2 與 2/26；Q67 密度 花岡2.7<玄武3.0<橄欖3.3<鐵隕7.9
       - 待裁圖（約 24 張）：表1(q1) 圖1(q3) q5選項 圖2圖3(q10) q16選項 圖4(q17) 圖5(q20)
         圖6圖7(g22) q23選項 圖8(q24) 圖9(g29) 圖10(q31) 圖11(q32) 圖12(q33) 圖13(q35) 圖14(q49)
         圖15圖16(q52) 圖17圖18(g55) 圖19圖20(g57) q57選項 圖21(q59) 圖22(q63) 圖23(q66)
