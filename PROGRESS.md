...
NEXT_ACTION: 【歷屆學測自然】110～102 九卷已完成。**接著做 101、100 兩卷就收工**（2026-09-02 Tony：各科都做到 100 年後的就好）（subj science；111 起自然 44 題約 96 分、110 以前 68 題 128 分，逐卷看卷首配分）。國英數社四科 109 卷 4,528 題已收齊（年份下限 90）。自然做完接會考數英社自（115-103）、基測（102-90）。流程照舊：
 ...
 (9) 歷屆學測（2026-09-01 Tony 新指示：「所有學測題另外做一個大項。進去可選哪一年哪一科直接做全部。
     ...
     【總進度表】（做完一項就把 ⬜ 改成 ✅，並補上卷數／題數）
       ...
       學測 自然：✅115（48 題/96 分）✅114（48 題/96 分）✅113（48 題/96 分）✅112（52 題/104 分）✅111（52 題/104 分）
                ...
                ✅102（68 題/136 分）⬜101 ⬜100（做到 100 為止，見下方 2026-09-02 Tony 範圍指示）
       ...
       【下一步：101、100 自然】PDF 已下載好放在 `<舊scratchpad>/dl/`（101自然試卷.pdf／101自然答案.pdf／
         100自然試卷.pdf／100自然答案.pdf，各 16 頁），raw.txt 也已用 pdftotext -layout 抽好在
         `<新scratchpad>/n101/raw.txt`、`n100/raw.txt`。做法完全照 102-110：讀卷首確認配分與選項數例外
         → 裁圖 → 分批寫解析 → build.py 對答案 → 註冊 exams.js → node test/test.js → stamp → push。
