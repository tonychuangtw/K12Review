...
for (const f of ['texts-social', 'texts-science', 'texts-math', 'texts-english', 'texts-chinese',
     ...
     判準：同一個 冊|課 裡，往前找到第一個不是承上題的題目，它要帶得動文章。 */
{
  ...
  const KNOWN_NO_DATA = ['揹', '譁', '縝', '靄', '譟', '靨', '鎚', '粿', '韉', '蹕']; // 筆順資料庫查無此字,前端已改顯示標楷體靜態字
   ...
   單元名對不上題庫的 lesson 就永遠不會出現在網站上，這是最容易犯又最難發現的錯。 */
{
  ...
  const WIDGETS = ['fracbar', 'fraccircle', 'fraccompare', 'placevalue', 'column',
                   ...
                   'accuracy', 'workgraph', 'fieldlines', 'energylevel', 'fission', 'greenhouse',
  ...
  // 畫面上不明顯但就是錯字。這段直接掃原始檔，之後不會再靜默出貨。
  {
    ...
    const files = ['js/data/lessons-math.js', 'js/data/lessons-science.js',
      ...
      'js/data/lessons-history.js', 'js/data/lessons-geography.js',
      'js/data/lessons-civics.js', 'js/widgets.js'];
    const hits = [];
    files.forEach(f => {
